...
class Curso
{
	...
	public function getAll($tipo = '')
	{
		...
		$sql = "SELECT " . $this->tabla . ".* FROM " . $this->tabla . " INNER JOIN com_escuelas_cursos ON " . $this->tabla . ".id = com_escuelas_cursos.curso WHERE " . $this->tabla . ".id > :id AND " . $this->tabla . ".estado = 1 AND com_escuelas_cursos.escuela = :escuela GROUP BY " . $this->tabla . ".id";
		$bind = array(
			':id' => '0',
...
/*
	...
	public function preinscribir($id, $usuario, $estado, $tipouser, $prerequisitos = 0)
	{
...
fclose($file);*/
	...
	public function reporteAlumnos_3($id, $estado)
	{
		...
		$sql = "SELECT core.id, core.nombre,core.ape1,core.ape2,core.email,core.dni,core.telefono,core.clave,core.genero,core.tipouser,reg.region,cocu.titulo AS cursos, cure.estado, cure.estadopago, cure.validprerequisitos, cure.idpago, cure.floworder, cocu.acred_pre, cocu.ciclo FROM com_registro core 
		JOIN com_cursos_registro cure on cure.usuario = core.id 
		JOIN com_cursos_2022 cocu on cocu.id = cure.curso
	...
	public function getAll_Pagos()
	{
		...
		$sql = "SELECT com_registro.nombre, com_registro.ape1, com_registro.ape2, com_registro.email, com_registro.telefono, com_pagos.* FROM com_pagos LEFT JOIN com_registro ON com_registro.id = com_pagos.usuario WHERE (com_pagos.tipopago = 2 AND com_pagos.estadopago > 0 AND com_pagos.estadopago < 3) OR com_pagos.tipopago = 1 GROUP BY com_pagos.id ORDER BY com_pagos.fecha DESC";
	...
	public function getAll_CursosPagos($idpago)
	{
		...
		$sql = "SELECT com_cursos_2022.titulo, com_cursos_2022.ciclo FROM com_pagos LEFT JOIN com_cursos_registro ON  com_cursos_registro.idpago = com_pagos.id LEFT JOIN com_cursos_2022 ON com_cursos_registro.curso = com_cursos_2022.id WHERE com_pagos.id = :idpago ORDER BY com_cursos_2022.id ASC";
		$bind = array(
			':idpago' => $idpago
		);
		$cont = $db->run($sql,$bind);
