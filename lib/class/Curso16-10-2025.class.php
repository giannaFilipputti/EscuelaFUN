...
class Curso
{
	...
	public function getAll($tipo = '', $presencial = 0)
	{
		...
		if (empty($this->escuela)) {
			...
			if ($presencial == 1) {
				$sql .= " AND " . $this->tabla . ".presencial = 1";
			}
		} else {
			$tipo = 'todos';
	...
	public function getAllByUser($usuario, $estado = 1)
	{
		...
		$sql = "SELECT " . $this->tabla . ".* FROM " . $this->tabla . " INNER JOIN com_cursos_registro ON com_cursos_registro.curso = " . $this->tabla . ".id WHERE " . $this->tabla . ".id > :id AND (" . $this->tabla . ".estado >= :estado OR " . $this->tabla . ".test= 1) AND com_cursos_registro.usuario = :usuario";
		$bind = array(
			':id' => 0,
			':usuario' => $usuario,
			':estado' => $estado
	...
	public function getUsuarios()
	{
		...
		$sql = "SELECT com_registro.*,com_regiones.region,GROUP_CONCAT(cocu.titulo) AS cursos
                ...
                LEFT JOIN com_cursos_registro cure on cure.usuario = com_registro.id 
                LEFT JOIN com_cursos_2022 cocu on cocu.id = cure.curso
	...
	public function reporteAlumnos()
	{
		...
		$sql = "SELECT core.id,core.nombre,core.ape1,core.ape2,core.email,core.fecnac,core.dni,reg.region,GROUP_CONCAT(cocu.titulo,'-',cocu.ciclo SEPARATOR ',') AS cursos FROM com_registro core 
	...
	{
		$db = Db::getInstance();
		$sql = "SELECT com_cursos_registro.*,  com_cursos_2022.id AS idC, com_cursos_2022.acred_prere,  com_cursos_2022.titulo,  com_cursos_2022.precio,  com_cursos_2022.precio1,  com_cursos_2022.precio2,  com_cursos_2022.precio3,  com_cursos_2022.acred_pre, com_cursos_2022.mailingID, com_cursos_2022.ciclo, com_cursos_2022.fecha, com_cursos_2022.plazo FROM com_cursos_registro INNER JOIN com_cursos_2022 ON com_cursos_registro.curso = com_cursos_2022.id WHERE com_cursos_registro.usuario = :usuario AND com_cursos_registro.estado = 0 AND com_cursos_registro.estadopago = 0";
		$bind = array(
			':usuario' => $usuario
		);
	...
	{
		$db = Db::getInstance();
		$sql = "SELECT com_cursos_registro.*,  com_cursos_2022.id AS idC, com_cursos_2022.acred_prere,  com_cursos_2022.titulo,  com_cursos_2022.precio,  com_cursos_2022.precio1,  com_cursos_2022.precio2,  com_cursos_2022.precio3,  com_cursos_2022.acred_pre FROM com_cursos_registro INNER JOIN com_cursos_2022 ON com_cursos_registro.curso = com_cursos_2022.id WHERE com_cursos_registro.usuario = :usuario AND com_cursos_registro.estado = 0 AND com_cursos_registro.validprerequisitos = 0";
		$bind = array(
			':usuario' => $usuario
		);
	...
	static function aceptarPrerequisitos($curso, $usuario, $data4)
	{
		...
		$db4->update('com_cursos_registro', $data4, 'curso = :curso AND usuario = :usuario', array(':curso' => $curso, ':usuario' => $usuario));
	}

	static function ampliarPlazo($curso, $usuario, $data4)
	{
	...
	static function updateIDPagoTransf($usuario, $idregistro, $idpago)
	{
		...
		$db4->update('com_cursos_registro', $data4, 'id = :id AND usuario = :usuario', array(':id' => $idregistro, ':usuario' => $usuario));
	}
	...
	static function updateFechasInicio($id, $usuario, $data4)
	{
		...
		$db->update('com_cursos_registro', $data4, 'id = :id AND usuario = :usuario', array(':id' => $id, ':usuario' => $usuario));
	}
	...
	public function preinscribir($id, $usuario, $estado, $tipouser, $prerequisitos = 0, $fecini = Null, $fecfin = Null)
	{
		...
		$verificar = Curso::checkInscritoCurso($id, $usuario);
		...
		if ($verificar == 0) {
			...
			//print_r($data1);
			$db1->insert('com_cursos_registro', $data1);
		}
	}
...
fclose($file);*/
	...
	static function subirTransferencia($idpago, $usuario, $valor, $ext, $nombre)
	{
		...
		Curso::enviarEmail($usuario, $fecha, 0, 2);
	}
	...
	static function activarCurso($curso, $usuario, $tipouser, $monto)
	{
		...
		$db->update('com_cursos_registro', $data, 'curso = :curso AND usuario = :usuario', array(':curso' => $curso, ':usuario' => $usuario));
	}
	...
	static function elimPreins($id, $usuario)
	{
		...
		$db->delete('com_cursos_registro', "id=:id AND usuario=:usuario AND estadopago = 0 AND estado = 0", array(':id' => $id, ':usuario' => $usuario));
	}
	...
	static function elimFormaPago($id, $usuario)
	{
		...
		$db0->update('com_cursos_registro', $data0, 'idpago = :id AND usuario = :usuario AND estadopago = 0', array(':id' => $id, ':usuario' => $usuario));
	}
	...
	public function updateJoinURL($usuario, $curso, $url)
	{
		...
		// $db->save('com_evento_registro', $data, 'evento = :reunion AND usuario = :usuario', array(':reunion' => $reunion,':usuario' => $usuario));

		$db->update('com_cursos_registro', $data, 'curso = :curso AND usuario = :usuario', array(':curso' => $curso, ':usuario' => $usuario));
	}


	public function reporteAlumnos_clinicas($id, $estado)
	{
		...
		$sql = "SELECT core.id, core.nombre,core.ape1,core.ape2,core.email,core.dni,core.telefono,core.clave,core.genero,core.tipouser,reg.region,cocu.titulo AS cursos, cure.porcentaje AS porcCurso FROM com_registro core 
	...
	public function reporteAlumnos_2022($id)
	{
		...
		JOIN com_cursos_registro_2022 cure on cure.usuario = core.id 
		JOIN com_cursos_2022 cocu on cocu.id = cure.curso
		LEFT JOIN com_regiones reg on reg.id = core.region
		WHERE cocu.id = :id
		GROUP BY core.id ORDER BY core.id ASC";
	...
	public function getAll_Presenciales()
	{

		$db = Db::getInstance();

		$sql = "SELECT id,titulo, ciclo, estado FROM com_presenciales ORDER BY id ASC";
	...
	public function getAll_CursosPagos($idpago)
	{
		...
		$sql = "SELECT com_cursos_2022.id, com_cursos_2022.titulo, com_cursos_2022.ciclo, com_cursos_2022.acred_pre, com_cursos_2022.mailingID, com_cursos_2022.fecha, com_cursos_2022.plazo FROM com_pagos LEFT JOIN com_cursos_registro ON  com_cursos_registro.idpago = com_pagos.id LEFT JOIN com_cursos_2022 ON com_cursos_registro.curso = com_cursos_2022.id WHERE com_pagos.id = :idpago ORDER BY com_cursos_2022.id ASC";
		$bind = array(
			':idpago' => $idpago
		);
		$cont = $db->run($sql, $bind);
	...
	static function actualizarAvance($user, $segundos_cur, $id_cur)
	{
		...
		if ($cont > 0) {
			...
			$db2->update('com_cursos_registro', $data2, "usuario = :usuario AND curso = :curso", array('usuario' => $user, 'curso' => $id_cur));
		} else {
			$porc = 0;
		}
