...
class Examen
{
	...
	public function getEstado()
	{
		...
		if ($cont == 0) {
			...
			if ($row_p[0]['estado'] == 0) {
				$fechaPlazo = strtotime(date('Y-m-d H:i:s') . "- 1 days");
				$mod_date = date("Y-m-d H:i:s", $fechaPlazo);

				$fechaini = $row_p[0]['fecini'];
				if ($mod_date > $fechaini) {
					$this->forzar_cierre = 1;
					$this->cerrarExam($row_p[0]['id']);
					$this->getEstado();
				} else {
					$this->plazo_examen = $row_p[0]['fecini'];
					return  1;
				}
	...
	public function getCertificadoValido($clave)
	{
		...
		$sql = "SELECT com_alumnos_exam.*, com_registro.nombre, com_registro.ape1, com_registro.ape2, com_certificaciones.titulo  FROM com_alumnos_exam INNER JOIN com_registro ON com_registro.id = com_alumnos_exam.alumno LEFT JOIN com_certificaciones ON com_alumnos_exam.modulo = com_certificaciones.id WHERE com_alumnos_exam.clave=:clave";
	...
	public function generateQR($codigo)
	{
		...
		$codeContents = 'https://www.charlasycursosjde.cl/qr.php?codigo=' . $codigo;
		$nameContents = $codigo;
