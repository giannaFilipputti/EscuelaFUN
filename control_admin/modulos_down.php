...
function startUpload(id, conditional)
{
	if(conditional.value.length != 0) {
		...
		if ($("#per_asi").is(":checked")) {  
           var per_asi = $('#per_asi').val();
        } else {
           var per_asi = "";
        }
		if ($("#per_med").is(":checked")) {  
           var per_med = $('#per_med').val();
        } else {
           var per_med = "";
        }
		if ($("#per_del").is(":checked")) {  
           var per_del = $('#per_del').val();
        } else {
           var per_del = "";
        }

		$('#'+id).uploadifySettings('scriptData', { 'titulo': remplazos($('#titulo').val()),'contenido': remplazos($('#contenido').val()),'per_asi': remplazos(per_asi), 'per_med': remplazos(per_med), 'per_del': remplazos(per_del), 'modulo': <?=$id?> });
		$('#'+id).uploadifyUpload();
	} else
...
$(document).ready(function() {
	...
	$("#fileUploadname3").uploadify({
        ...
        'script'    : '<?php echo $baseURL;?>mod_down/uploadify.php',
        'cancelImg' : '<?php echo $baseURL;?>plugins/uploadify/cancel.png',
        'folder': '<?php echo $baseURL;?>mod_down',
		'multi': false,
		'buttonText'  : 'Seleccionar Archivo',
	    'fileExt'     : '*.pdf;*.PDF;*.zip;*.ZIP',
        'fileDesc'    : 'Web Image Files (.PDF; .ZIP)',
		'method' : 'post',
		'displayData': 'percentage',
 
		onAllComplete: function (event, queueID, fileObj, response, data) {
			//alert(data);
			$.post("modulos_down_listado.php?id=<?=$id?>",   function(data1){            
...
<body class="twoColLiqLtHdr">
    ...
    <div id="container"> 
      ...
      <!-- DESDE AQUI CONTENIDO -->
        <h1>Descargas del Modulo: <?php echo $mod->row[0]['titulo'];?></h1>
        <div class="box">
        <h2>Agregar Descargas al Modulo </h2>
        ...
        <div id="imagenes">
        <h2>Descargas</h2>
        <?php include('modulos_down_listado.php');?>
