...
// $sql = "SELECT * FROM com_cursos_mod_cap WHERE id=".$id."";
...
function startUpload(id, conditional)
{
	if(conditional.value.length != 0) {
		...
		$('#'+id).uploadifySettings('scriptData', { 'titulo': remplazos($('#titulo').val()),'contenido': remplazos($('#contenido').val()),'tipo': remplazos($('#tipo').val()),'ubicacion': remplazos($('#ubicacion').val()), 'url': remplazos($('#url').val()), 'capitulo': <?=$id?> });
		$('#'+id).uploadifyUpload();
	} else
...
$(document).ready(function() {
	...
	$("#fileUploadname3").uploadify({
        ...
        'script'    : '<?php echo $baseURL;?>cap_down/uploadify.php',
        'cancelImg' : '<?php echo $baseURL;?>plugins/uploadify/cancel.png',
        'folder': '<?php echo $baseURL;?>cap_down',
		'multi': false,
		'buttonText'  : 'Seleccionar Archivo',
	    'fileExt'     : '*.pdf;*.PDF;*.zip;*.ZIP;*.jpg;*.JPG;*.PNG;*.png;*.gif;*.GIF',
        'fileDesc'    : 'Web Image Files (.PDF; .ZIP; .PNG; .GIF; .JPG)',
		'method' : 'post',
		'displayData': 'percentage',
 
		onAllComplete: function (event, queueID, fileObj, response, data) {
			//alert(data);
			$.post("capitulos_cap_down_listado.php?id=<?=$id?>",   function(data1){            
...
<body class="twoColLiqLtHdr">
    ...
    <div id="container"> 
      ...
      <!-- DESDE AQUI CONTENIDO -->
        <h1>Descargas del Capitulo: <?php echo $capitulo->row[0]['titulo'];?></h1>
        <div class="box">
        <h2>Agregar Descargas al Capitulo </h2>
        
        <label><span>Nombre: </span>
          <input type="text" name="titulo" id="titulo" size="20"></label>
          <label><span>Descripcion: </span>&nbsp;</label>
          <label><span>Tipo</span><select name="tipo" id="tipo">
          <option value="descarga">Descarga</option>
          <option value="url">Link</option>          
          </select></label>
          <label><span>Ubicacion</span><select name="ubicacion" id="ubicacion">
          <option value="anexos">Anexos</option>
          <option value="biblio">Bibliografia</option>          
          </select></label>
          <div>
          <label><span>URL</span><input type="text" name="url" id="url" value="http://" /></label>
          </div>
         <textarea id="contenido" name="contenido" rows="5" cols="50"></textarea>
        
        
            <div>si el tipo es "Link" suba la imagen que acompaña al link, si es una descarga, suba el archivo</div>
        ...
        <div id="imagenes">
        <h2>Descargas</h2>
        <?php include('capitulos_cap_down_listado.php');?>
