...
class Categoria
{
	...
	private function getOrden($tabla='com_categorias')
    {
	...
	{
				$db = Db::getInstance();
				$sql = "SELECT * FROM com_categorias WHERE id > :id ORDER BY orden DESC";
    			$bind = array(
        		':id' => '0'
    			);
				...
				if ($cont == 0) {
				   ...
				   foreach($row_p as $row_p1) {
						...
						$this->categoria_esp[$conty] = $row_p1['categoria_esp'] ;
						$this->categoria_eng[$conty] = $row_p1['categoria_eng'] ;
